...
Modified version:
    - Early stopping and checkpointing now use validation ROC-AUC instead of accuracy.
"""
...
parser.add_argument("--save_prefix", type=str, default="Exotropia_MobileNetV3-Mixup")
...
# Multi-seed loop
# ---------------------------
#seeds = [42, 35, 28, 14, 7, 53, 65, 78, 86, 98]
for seed in args.seeds:
    ...
    # ---------------------------
    # Training with Mixup + Early Stopping (AUC-based)
    # ---------------------------
    best_val_auc = 0.0
    patience_counter = 0
    ...
    for epoch in range(args.epochs):
        ...
        for imgs, labels in train_loader:
            ...
            imgs, targets_a, targets_b, lam = mixup_data(imgs, labels, alpha=args.mixup_alpha, device=device)
            optimizer.zero_grad()
            outputs = model(imgs)
        ...
        if val_auc > best_val_auc:
            ...
            patience_counter = 0
            best_model_wts = deepcopy(model.state_dict())
            torch.save(best_model_wts, f"best_seed_Exotropia_MobileNetV3_Mixup_{seed}.pth")
        else:
            patience_counter += 1
            print(f"Epoch {epoch+1}/{args.epochs} "
                  f"Loss: {avg_loss:.4f} | "
    ...
    # Test Evaluation
    # ---------------------------
    model.load_state_dict(torch.load(f"best_seed_Exotropia_MobileNetV3_Mixup_{seed}.pth"))
    model.eval()
    ...
    json_filename = f"Mixup_Exotropia_MobileNetV3_{seed}.json"
